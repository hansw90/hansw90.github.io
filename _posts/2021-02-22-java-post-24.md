...
#### 2. TypeHandler 란 무엇인가?
TypeHandler는 mybatis가 PreparedStatement에 파라미터를 설정하고 ResultSet에서 값을 가져올때 적절한 자바 타입의 값을 가져오거나 INSERT시에 PreparedStatement에
적절한 자바 타입의 값을 set할때 사용이 된다.  
...
- 동일한 sql 구문을 반복 실행 한다면 preparedState가 성능면에서 굉장히 빠르다
-
```